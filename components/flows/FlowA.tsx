  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Card>
        ...
        {[
          { icon: '💳', title: 'Rewards card eligible',     detail: 'Your Sapphire card earns 3x on utilities. Bill qualifies as a utility merchant.', value: '+444 points on $148' },
          { icon: '🛡️', title: 'Cash flow protected',       detail: 'Checking stays at $4,210 — well above your $1,200 floor. Rent on Jun 14 unaffected.', value: '$3,010 above floor after rent' },
          { icon: '🎯', title: 'Goal velocity maintained',  detail: 'Keeping checking intact means your $500 goal transfer on Jun 8 executes without risk.', value: 'Down payment on track for Dec 2026' },
        ].map((item, i) => (
          <div key={i} className={`flex gap-3 py-3 ${i < 2 ? 'border-b border-hairline-soft' : ''}`}>
            <div className="w-9 h-9 rounded-md bg-soft flex items-center justify-center flex-shrink-0 text-lg">{item.icon}</div>
...
function ScreenConfirm({ ruleEnabled, onToggleRule, onConfirm, onPayFromChecking }: {
  ruleEnabled: boolean; onToggleRule: (val: boolean) => void
  onConfirm: () => void; onPayFromChecking: () => void
}) {
  return (
    <div className="flex flex-col gap-4">
      <div>
      ...
      <div className="flex flex-col gap-3 mt-2">
        <Button variant="primary" label={ruleEnabled ? 'Confirm + save rule' : 'Confirm this payment'} onClick={onConfirm} />
  ...
  return (
    <div className="flex flex-col min-h-screen bg-canvas">
      <div className="flex items-center justify-between px-container pt-12 pb-4">
        <button onClick={onBack} className="flex items-center gap-1.5 text-muted" style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px 0' }}>
